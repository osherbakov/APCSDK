...
Namespace Diacom.APCStates
    ...
    '''</summary>
    Friend Class RecordFileModule : Inherits EventProcessingModule
        ...
        Public Overrides Sub StartOperation()
            DBGLN(m_Line, "Module: RecordFile: Start: " & m_FileName)
            m_Line.spCommand.SendRecordFileCommand(m_SourceLine.LineReference, m_Line.LineReference, m_FileName, m_Cutoff)
            m_TimerError = m_TimeoutError
            m_Active = True
        End Sub

        Public Overrides Sub FinishOperation()
            DBGLN(m_Line, "Module: RecordFile: Finish: " & m_FileName)
            If m_Active = False Then
                MyBase.FinishOperation()
            ElseIf m_StopSent = False Then
                DBGLN(m_Line, "Module: RecordFile: Finish: Send Reset")
                m_Line.spCommand.SendResetCommand(m_SourceLine.LineReference, m_Line.LineReference)
                m_StopSent = True
            End If
        End Sub
        Public Overrides Function ToString() As String
            Return "Module: RecordFile:" & " File: " & m_FileName
        End Function
    End Class
End Namespace
